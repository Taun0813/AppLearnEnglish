...
class VocabularyTab(QWidget):
    def __init__(self):
        super().__init__()
        self.setStyleSheet("QLabel#title { font-size: 24px; font-weight: bold; }")  # Retain title styling
        ...
        self.word_list_widget = QListWidget()
        self.start_quiz_button = QPushButton("Bắt đầu Luyện Tập")  # Updated button text
        self.start_quiz_button.clicked.connect(self.start_quiz)
        ...
        self.result_label = QLabel("")
        self.submit_button = QPushButton("Kiểm Tra")  # Updated button text
        self.submit_button.clicked.connect(self.check_answer)
        self.skip_button = QPushButton("Câu Khác")  # Updated button text
        self.skip_button.clicked.connect(self.skip_word)
    ...
    def set_quiz_visible(self, visible):
        for widget in [self.mode_label, self.meaning_label, self.input_label,
                       self.user_input, self.result_label, self.submit_button, self.skip_button]:
            widget.setVisible(visible)
        self.word_list_widget.setVisible(not visible)  # Keep list visibility logic
        self.start_quiz_button.setVisible(not visible)
    ...
    def skip_word(self):
        self.show_quiz()  # Keep skip logic
